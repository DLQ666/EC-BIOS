...
for %%i in (MMTool.exe UEFIFind.exe UEFIExtract.exe DrvVer.exe FindVer.exe HexFind.exe mcodeinfo.exe) do (
	if not exist %%i (
		echo ! %%i not found !
		pause
		exit
...
(uefifind body count 49006E00740065006C00AE004400650073006B0074006F007000200042006F00610072006400 bios.bin>nul ||uefifind body count 49006E00740065006C00........4400650073006B0074006F007000200042006F00610072006400 bios.bin>nul) && echo File Intel Desktop Board BIOS is not supported! && goto exit1
uefifind body count 4153555342....24 bios.bin>nul && set asus=1
for /f "tokens=1" %%o in (' uefifind body list 244649440.......................................................303.003. bios.bin') do %mmt% /e %%o tmp\aptiov.tmp
rem 00AA24464944
if exist tmp\aptiov.tmp for /f %%a in ('findver "" 24464944 33 00 1 2 tmp\aptiov.tmp') do set aa=%%a
if %aa%==0 (
	uefifind body count 494E5359444548324F>nul bios.bin && echo InsydeH2O BIOS not supported. && pause && goto exit1
	uefifind body count 50686F656E697820534354>nul bios.bin && echo PhoenixSCT BIOS not supported. && pause && goto exit1
	echo Unknow Versiom AMI Aptio && pause && goto exit1
)
if %aa%==5 echo Platform BIOS AMI Aptio %aa%
if %aa%==4 echo Platform BIOS AMI Aptio %aa%
...
uefifind body count 304147455341 bios.bin>nul || set m7=1

:cpu_pk


for /f "tokens=1" %%f in ('uefifind body list 5F4649545F202020..0000000001 bios.bin') do (
	if %%f neq 00000000-0000-0000-0000-000000000000 (
		echo Found _FIT_ in GUID %%f
		set fit=%%f
	) else (
		echo Found _FIT_ in PADDING
		set fit=PADDING
	)
)
...
rem AMD AGESA
for /f "tokens=1" %%b in ('uefifind body list 00000000..21214147455341  bios.bin') do (
	echo Found EFI AMD AGESA GUID %%b
	%mmt% /e %%b tmp\Agesa.tmp
)
for /f "tokens=*" %%v in ('findver "" 21214147455341 7 00 25 1 tmp\Agesa.tmp') do set agesa=%%v
...
for /f "tokens=1" %%b in ('uefifind body list 41004D0044002D0052004100490044003A002000440072006900760065007200200045006E00740072007900 bios.bin') do (
...
if exist tmp\cpuffs.ffs if exist MCE.dat if %m7% neq 0 mCodeInfo -scan tmp\cpuffs.tmp
set mea=0
if %m7% neq 0 if exist MEA MEA\MEA.exe bios.bin -ubupre -adir
if %errorlevel%==9 set mea=1
if %mea%==1 for /f "tokens=1-5" %%a in ('MEA\MEA.exe bios.bin -extr -adir') do (set mead=     FW %%a of scanned BIOS File   %%c %%d)

if %aa%==5 (
	echo;
	echo  This BIOS on Aptio 5 platform, known issues:
	echo   - It is not supported by the update files in the GUID A0327FE0-1FDA-4E5B-905D-B510C45A61D0
...
if %m7%==7 (
	echo 7 - Update Intel CPU MicroCode
	drvver tmp\cpuffs.tmp
	if exist MCE.dat mCodeInfo -scan tmp\cpuffs.tmp
)

if exist tmp\agesa.tmp (
	echo   AMD AGESA CPU Support
	echo      AMD AGESA %agesa%
)
...
echo i - Versions, HomePages and etc.
echo 0 - Exit
...
if %enter%==7 if %m7%==7 goto cpu
if %enter%==8 if %m8%==8 goto rg
if %mea%==1 if %enter%==9 (
	MEA\MEA.exe bios.bin -ubu -adir
	pause
	goto mn1
)
...
if %s1151%==1 echo 9 - Update to v15.2.0.2754
if %s1151%==1 echo 10 - Update to v15.5.0.2760
...
if %s1151%==1 if %ec%==9 (set v=Modules\irst\15_2) && goto prcs
if %s1151%==1 if %ec%==10 (set v=Modules\irst\15_5) && goto prcs
...
if %%a neq A0327FE0-1FDA-4E5B-905D-B510C45A61D0 (
	(hexfind 16000013027C75D59906D9E0118D788DE44824019B08 tmp\irst_%%a.tmp>nul && set depx=Modules\irst\depex.sct) || set depx=0
	(hexfind 16000013028047E2AE1145234FA028EB8204D4829C08 tmp\irst_%%a.tmp>nul && set depx=Modules\irst\depex1.sct) || set depx=0
...
echo 1 - Update to v3.8.0.1029
echo   Optimal for X99
echo 2 - Update to v4.5.0.1012 SATA and sSATA/v4.3.0.1018 SCU
echo 0 - Skip
echo;
:rste1
set /p ec=Enter number:
...
if %ec%==2 (set v=Modules\irst\4_5) && goto prcse
...
if %aa%==4 if %lanrr%==1 echo     - OROM LAN Realtek %rlo%
...
if %s2011v3%==1 if %bacl%==1 if %bage%==1 set fefi=%sdl%\efi\E6604X3.efi
:x99
set depx=0
...
if not errorlevel 1 (set did=8169) && call :romg
...
(%sdl%\romhdr -v 0x%vid% -d 0x%did% -i %lanrom% -o tmp\lanrom.tmp>nul && echo Ok!) || echo Error!
...
:romu5e
<nul set /p TmpStr=* Update EFI %brend% GUID %oguid% ...
%mmt% /r /l %fefi% %oguid%
if %errorlevel%==0 (echo Ok!) else (echo Error!)
...
if %aa%==5 (
	%udk%\GenSec tmp\ffspe32.tmp tmp\ffsui.tmp %vers% -o tmp\ffspe32ui.tmp
	%udk%\LzmaCompress -e -q tmp\ffspe32ui.tmp -o tmp\ffslzmape32ui.tmp
	%udk%\GenSec -s EFI_SECTION_GUID_DEFINED -g EE4E5898-3914-4259-9D6E-DC7BD79403CF -r PROCESSING_REQUIRED tmp\ffslzmape32ui.tmp -o tmp\ffscomp.tmp
)
if %depx%==0 (
...
cls
echo;
echo     Current version - GUID %cguid%
mcodeinfo -info tmp\cpuffs.tmp
if %fit% neq 0 echo  _FIT_ in GUID %fit% && mcodeinfo -fit bios.bin
...
set cpu2011v=6E4-428, 6E2-20D, 6D7-710, 6D6-61A, 6D5-513
...
if %s2011%==1 (
	...
	echo     6E4-428, 6E2-20D, 6D7-710, 6D6-619, 6D5-513
...
if exist mce.dat echo s - Search for available microcode in DB.
echo 9 - View CPU Microcode Patch list
echo 0 - Exit to Main Menu
echo;
:cpu1
set /p ec=Enter number:
...
if /I %ec%==m goto mcg
if exist mce.dat if /I %ec%==s goto sdb
if %ec%==9 start %cvcpu% mmtool bios.bin /p && goto cpu1
goto cpu1

:sdb
set /p str1=Enter CPUID, example 0306C3 :^>
if not defined str1 goto cpu
set /p str2=Enter Platform ID, example 32 :^>
if not defined str2 goto cpu
set fstr=00%str1%000000%str2%
echo,
findstr /B /I /C:%fstr% MCE.dat || echo Not found!
pause
goto cpu
...
echo %fmc%
mcodeinfo -info %fmc%
mcodeinfo -check %fmc% || pause && goto cpu
...
if /I %ec%==y (
	...
	%udk%\GenFFS -s -t EFI_FV_FILETYPE_RAW -g %cguid% -i %fmc% %mpdt% -o tmp\usermc.ffs
...
if defined mc1 (mcodeinfo -check Modules\mCode\%mc1% || pause && goto cpu) && (set mc1=-i Modules\mCode\%mc1%)
if defined mc2 (mcodeinfo -check Modules\mCode\%mc2% || pause && goto cpu) && (set mc2=-i Modules\mCode\%mc2%)
echo Generate FFS files Microcode
%udk%\GenFFS -s -t EFI_FV_FILETYPE_RAW -g %cguid% %mc1% %mc2% %mpdt% -o tmp\mCode.ffs
set modcpu=tmp\mCode.ffs

:cpus
echo Found %c% module(s).
...
%mmt% /e %cguid% tmp\cpuffs.tmp
for /f "tokens=1" %%m in ('uefifind header list 72850817000000000000000000000001 bios.bin') do (
	echo Restore "Empty" module...
	%mmt% /r %%m tmp\cpu_zero.tmp
)
mcodeinfo -info tmp\cpuffs.tmp
if %fit% neq 0 echo  _FIT_ in GUID %fit% && mcodeinfo -fit bios.bin
if %fit% neq 0 if defined mc1 if defined mc2 (
	echo,
	echo    Attention!!!
	echo Check the Address in the _FIT_
	echo Address 01 mCode + Size 01 mCode = Address 02 mCode
	for /f "tokens=5" %%a in ('mCodeInfo -fit bios.bin') do set amc=0x%%a && goto n1
	:n1
	for /f "skip=3 tokens=10" %%s in ('mCodeInfo -info tmp\cpuffs.tmp') do set smc=0x%%s && goto n2
	:n2
	<nul set /p TmpStr=  %amc% + %smc% = 0x
	mcodeinfo -mcalc %amc% %smc%
	echo,
)
pause
goto mn1
...
for /f "tokens=1,2" %%a in ('uefifind body list 426F6F74204167656E7420434C bios.bin') do (
	...
	for /f "eol=; tokens=1,2" %%d in (%sdl%\DevID_List_iCL.txt) do (
		hexfind 504349528680%%d tmp\BACL.lom>nul && echo Device ID %%e && echo Device ID %%e>>#%finfo% && %sdl%\romhdr -v 0x8086 -d 0x%%e -i %sdl%\IntlOromCL.LOM -o _BootAgentCL_%%e.lom>nul && echo Create _%fbacl%_%%e.lom && echo Create _%fbacl%_%%e.lom>>#%finfo% && del tmp\bacl.lom && echo;
		)
	)
)
...
for /f "tokens=1,2" %%a in ('uefifind body list 426F6F74204167656E74204745 bios.bin') do (
	...
	for /f "eol=; tokens=1,2" %%d in (%sdl%\DevID_List_iGE.txt) do (
		hexfind 504349528680%%d tmp\BAGE.lom>nul && echo Device ID %%e && echo Device ID %%e>>#%finfo% && %sdl%\romhdr -v 0x8086 -d 0x%%e -i %sdl%\IntlOromGE.LOM -o _BootAgentGE_%%e.lom>nul && echo Create _%fbage%_%%e.lom && echo Create _%fbage%_%%e.lom>>#%finfo% && del tmp\bage.lom  && echo;		
	    )
	for /f "eol=; tokens=1,2" %%d in (%sdl%\DevID_List_iCL.txt) do (
		hexfind 504349528680%%d tmp\BAGE.lom>nul && echo Device ID %%e && echo Device ID %%e>>#%finfo% && %sdl%\romhdr -v 0x8086 -d 0x%%e -i %sdl%\E1562X3.LOM -o _BootAgentGE_%%e.lom>nul && echo Create _%fbage%_%%e.lom && echo Create _%fbage%_%%e.lom>>%finfo% && del tmp\bage.lom  && echo;		
	    )
    )
)
echo;
